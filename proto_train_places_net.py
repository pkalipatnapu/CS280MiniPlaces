...
parser.add_argument('--use_proto_file', type=bool, default=False,
    help='If set, CNN structure will be taken from proto files')
...
def minialexnet(data, split, labels=None, train=False, param=learned_param,
    ...
    if not args.use_proto_file:
    	...
    	if split == 'val':
	    return "third.prototxt"
	if split == 'test':
	    return "fourth.prototxt"
	if split == 'train':
   	    if with_labels:
	    	return "first.prototxt"
	    else:
	    	return "second.prototxt"
...
def miniplaces_net(source, split, train=False, with_labels=True):
    ...
    return minialexnet(data=places_data, split=split, labels=places_labels, train=train,
...
def train_net(with_val_net=False):
    train_net_file = miniplaces_net(get_split('train'), 'train', train=True)
    ...
    # displayed.
    if with_val_net:
        val_net_file = miniplaces_net(get_split('val'), 'val', train=False, val=True)
    else:
        val_net_file = None
...
def eval_net(split, K=5):
    ...
    if split == 'val':
        test_net_file = miniplaces_net(split_file, split, train=False, with_labels=False)
    else:
        test_net_file = miniplaces_net(split_file, split, train=False, with_labels=False)
